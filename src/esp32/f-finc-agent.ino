...
#include <HTTPUpdate.h> // <-- THÊM THƯ VIỆN CHO OTA
...
// !!! THAY ĐỔI URL NÀY thành địa chỉ máy chủ của bạn !!!
const char* firmware_manifest_url = "http://your-server.com/firmware/manifest.json"; 
...
void checkFirmwareUpdate() {
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        if (newVersion > FIRMWARE_VERSION) {
            ...
            WiFiClient client; // Cần thiết cho httpUpdate
        ...
        Serial.printf("OTA: Failed to download manifest, HTTP error: %s\n", http.errorToString(httpCode).c_str());
    }
    http.end();
}
